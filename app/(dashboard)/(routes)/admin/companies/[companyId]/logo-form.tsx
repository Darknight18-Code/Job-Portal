...
const formSchema = z.object({
    logo: z.string().min(1, "Logo is required").optional().or(z.literal("")),
});
...
const LogoForm = ({ initialData, companyId }: LogoFormProps) => {
    ...
    const handleRemove = async () => {
        ...
        const publicId = extractPublicId(logo);
        setIsDeleting(true);

        try {
            await axios.post("/api/cloudinary/delete", { publicId });

            // update in MongoDB by setting logo to ""
            await axios.patch(`/api/companies/${companyId}`, { logo: "" });
